...
use Sineflow\PdfParser\Parser;
use Sineflow\PdfParser\Document;
...
class PdfAnkiImportService
{
    ...
    /**
     * Extrair texto do PDF usando a biblioteca sineflow/pdf-parser
     */
    private function extractTextFromPdf($filePath)
    {
        try {
            // Tentar usar sineflow/pdf-parser
            if (class_exists('Sineflow\PdfParser\Parser')) {
                $parser = new Parser();
            ...
            Log::warning('Erro com sineflow parser: ' . $e->getMessage());
        }

        // Fallback: usar pdftotext via shell (se disponível no servidor Linux)
        try {
            ...
            Log::warning('Erro com pdftotext: ' . $e->getMessage());
        }

        throw new \Exception('Nenhum método disponível para extrair texto do PDF. Instale sineflow/pdf-parser ou pdftotext.');
    }
