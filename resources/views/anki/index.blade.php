<x-app-layout>
    ...
    <div class="py-4 bg-alura-dark">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <!-- Estatísticas Rápidas -->
            <div class="grid grid-cols-1 md:grid-cols-4 gap-2 mb-4">
                <div class="alura-card rounded shadow p-3 border-l-4 border-blue-500">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-alura-text-muted text-xs font-medium">Total de Cards</p>
                            <p class="text-xl font-bold text-alura-text mt-0.5">{{ $totalCards }}</p>
                        </div>
                ...
                <div class="alura-card rounded shadow p-3 border-l-4 border-green-500">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-alura-text-muted text-xs font-medium">Cards Estudados</p>
                            <p class="text-xl font-bold text-alura-text mt-0.5">{{ $cardsStudied }}</p>
                        </div>
                ...
                <div class="alura-card rounded shadow p-3 border-l-4 border-orange-500">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-alura-text-muted text-xs font-medium">Prontos para Revisar</p>
                            <p class="text-xl font-bold text-alura-text mt-0.5">{{ $cardsDueReview }}</p>
                        </div>
                ...
                <div class="alura-card rounded shadow p-3 border-l-4 border-purple-500">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-alura-text-muted text-xs font-medium">Decks Ativos</p>
                            <p class="text-xl font-bold text-alura-text mt-0.5">{{ count($decksWithProgress) }}</p>
                        </div>
            ...
            @if($decksWithProgress->isNotEmpty())
                <div class="mb-4">
                    <h2 class="text-lg font-bold text-alura-text mb-3">Meus Decks</h2>
            ...
            @else
                <div class="alura-card rounded shadow p-6 text-center">
                    <svg class="w-12 h-12 text-alura-text-muted mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    <h3 class="text-sm font-semibold text-alura-text mb-1">Nenhum Deck encontrado</h3>
                    <p class="text-alura-text-muted text-xs mb-3">Você ainda não tem decks de Anki. Escolha uma opção abaixo:</p>
                    <div class="flex gap-2 justify-center flex-wrap">
                        <a href="{{ route('anki.import-page') }}" class="inline-block bg-alura-accent hover:bg-alura-accent-hover text-white font-semibold py-1.5 px-4 rounded text-xs transition-colors">
                            🎴 Importar
                        </a>
                        <a href="{{ route('courses.index') }}" class="inline-block bg-alura-card hover:bg-alura-darker text-alura-text font-semibold py-1.5 px-4 rounded text-xs transition-colors border border-alura-accent/30">
                            ← Voltar
                        </a>
                    </div>
                </div>
