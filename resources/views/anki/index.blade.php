    ...
    <div class="py-12 bg-alura-dark">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @if($decksWithProgress->isNotEmpty())
                <div class="mb-12">
                    <h2 class="text-2xl font-bold text-alura-text mb-6">Meus Decks</h2>
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        @foreach($decksWithProgress as $item)
                            <div class="alura-card rounded-lg shadow-lg overflow-hidden hover:shadow-xl transition-shadow">
                                <div class="bg-gradient-to-r from-alura-accent to-blue-600 px-6 py-4">
                                ...
                                <div class="p-6">
                                    ...
                                    <div class="mb-6">
                                        <div class="flex justify-between items-center mb-2">
                                            <span class="text-sm font-medium text-alura-text-muted">Progresso</span>
                                            <span class="text-sm font-bold text-alura-accent">{{ $item['progress_percentage'] }}%</span>
                                        </div>
                                        <div class="w-full bg-alura-darker rounded-full h-2">
                                            <div class="bg-alura-accent h-2 rounded-full transition-all" style="width: {{ $item['progress_percentage'] }}%"></div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="space-y-2 mb-6">
                                        <div class="flex justify-between text-sm">
                                            <span class="text-alura-text-muted">Total de Cards:</span>
                                        ...
                                        <div class="flex justify-between text-sm">
                                            <span class="text-alura-text-muted">Prontos para Revisar:</span>
                                    ...
                                    <a href="{{ route('anki.study', $item['deck']) }}" 
                                       class="block w-full bg-alura-accent hover:bg-alura-accent-hover text-white font-semibold py-2 px-4 rounded-lg text-center transition-colors disabled:opacity-50">
                                        Estudar Deck
                                    </a>
                                </div>
                            </div>
            ...
            @else
                <div class="alura-card rounded-lg shadow p-12 text-center">
                    <svg class="w-16 h-16 text-alura-text-muted mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    <h3 class="text-lg font-semibold text-alura-text mb-2">Nenhum Deck encontrado</h3>
                    <p class="text-alura-text-muted mb-6">Você ainda não tem decks de Anki. Escolha uma opção abaixo:</p>
                    <div class="flex gap-4 justify-center flex-wrap">
                        <a href="{{ route('anki.import-page') }}" class="inline-block bg-alura-accent hover:bg-alura-accent-hover text-white font-semibold py-2 px-6 rounded-lg transition-colors">
                            🎴 Importar de Pastas
                        </a>
                        <a href="{{ route('courses.index') }}" class="inline-block bg-alura-card hover:bg-alura-darker text-alura-text font-semibold py-2 px-6 rounded-lg transition-colors border border-alura-accent/30">
                            ← Voltar aos Cursos
                        </a>
                    </div>
                </div>
