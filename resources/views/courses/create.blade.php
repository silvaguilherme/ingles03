                    ...
                    <form method="POST" action="{{ isset($course) ? route('courses.update', $course) : route('courses.store') }}">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="title" id="title" 
                                   ...
                                   value="{{ $course->title ?? old('title') }}" required>
                            @error('title')
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="description" id="description" rows="5"
                                      class="mt-1 block w-full rounded-md border border-gray-300 shadow-sm px-4 py-2 text-black placeholder-gray-400">{{ $course->description ?? old('description') }}</textarea>
                            @error('description')
                                <span class="text-red-600 text-sm">{{ $message }}</span>
                            @enderror
                        </div>
